...
#ifdef DEBUG_ADVANCED_MALLOC_FREE
    printf("DEBUGA - set_dead_or_alive_single_static 0 - world_size=%ld, world_size * (world_size + 1)=%ld\n", world_size, world_size * (world_size + 1));
#endif
...
#pragma omp barrier
    for (long long i = 0; i < world_size * (world_size + 2); i++) {
        if (i % 16 == 0)
            printf("DEBUGB - set_dead_or_alive_single_static 1 - %08X: ", (unsigned int) i);
        printf("%02X ", world[i]);
        if (i % 16 == 15)
            printf("\n");
    }
...
#ifdef DEBUG_ADVANCED_MALLOC_FREE
        printf("DEBUGA - set_dead_or_alive_single_static 4 - i/world_size=%03lld/%ld x=%03ld, y=%03ld sum=%d\n", i, world_size, x, y, sum);
#endif
        // default is: cell will die
        world_next[i] = DEAD;
        int number_of_dead_neighbours = sum / DEAD; // 8-sum/255
...
#ifdef DEBUG1
    if (debug_info > 0)
        printf("DEBUG1 - evolution_static_single - BEGIN - mpi_rank=%d/%d, world_size=%ld\n", mpi_rank, mpi_size, world_size);
#endif
    unsigned char *world_local_actual = *world_local;
    // allocate memory for the next state
    unsigned char *world_local_next = (unsigned char *) malloc(world_size * (local_size + 2) * sizeof(unsigned char));
...
#ifdef DEBUG2
            if (debug_info > 1)
                printf("DEBUG2 - evolution_static_single 0 - mpi_rank=%d/%d, omp_rank=%d/%d, iteration_step=%d/%d\n", mpi_rank, mpi_size, omp_get_thread_num(), omp_get_max_threads(), iteration_step, number_of_steps);
#endif
...
#ifdef DEBUG2
                    if (debug_info > 1)
                        printf("DEBUG2 - evolution_static_single 1 - snap written mpi_rank=%d/%d, omp_rank=%d/%d, iteration_step=%d/%d\n", mpi_rank, mpi_size, omp_get_thread_num(), omp_get_max_threads(), iteration_step, number_of_steps);
#endif
                }
                // pointers swap to reuse allocated world_local and world_local_next for next iteration
                unsigned char *temp = world_local_actual;
...
#ifdef DEBUG2
            if (debug_info > 1)
                printf("DEBUG2 - evolution_static_single 2 - mpi_rank=%d/%d, omp_rank=%d/%d, iteration_step=%d/%d\n", mpi_rank, mpi_size, omp_get_thread_num(), omp_get_max_threads(), iteration_step, number_of_steps);
#endif
        }
    }
#ifdef DEBUG2
    if (debug_info > 1)
        printf("DEBUG2 - evolution_static_single 3 - mpi_rank=%d/%d, omp_rank=%d/%d\n", mpi_rank, mpi_size, omp_get_thread_num(), omp_get_max_threads());
#endif
    // if we had an odd swap, copy all the matrix
    if (number_of_steps % 2 == 1)
        for (long long i = world_size; i < world_size * (local_size + 1); i++)
...
#ifdef DEBUG2
    if (debug_info > 1)
        printf("DEBUG2 - evolution_static_single END - mpi_rank=%d/%d, omp_rank=%d/%d\n", mpi_rank, mpi_size, omp_get_thread_num(), omp_get_max_threads());
#endif
    return t_io;
}
...
void evolution_static(const char *filename, int number_of_steps, int number_of_steps_between_file_dumps, int *argc, char **argv[], int csv_output, int debug_info) {
    ...
    if (mpi_rank == 0) {
        ...
        if (directoryname == NULL) {
            perror("Unable to allocate buffer for directoryname in evolution_static\n");
            MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
        }

...
#SBATCH --output=scale_omp_epyc_5_job_%j.out
...
#SIZE=10000
      ...
      for threads in {64..1}; do
        ...
        export OMP_NUM_THREADS=$threads
        {
          mpirun -n 5 --map-by node --bind-to socket --report-bindings gameoflife.x -r -f pattern_random$SIZE.pgm -n $STEPS -e "$TYPE" -s "$SNAPAT" -q

...
for REP in {1..10}; do
  for LIB in oblas oblas_optimized mkl_optimized mkl blis blis_optimized; do
  csvname=dgemm_"$LIB"_epyc_$(hostname)_$now.csv
  ...
  #echo "$csvname $(hostname) $now">"$csvname"
    ...
    for SIZE in {30000..2000..500}; do
      echo dgemm_epyc_job_"$SLURM_JOB_ID".out$'\t'"$csvname"$'\t'"$(hostname)"$'\t'rep "$REP" size "$SIZE" OMP_PLACES=$OMP_PLACES OMP_PROC_BIND=$OMP_PROC_BIND OMP_NUM_THREADS=$OMP_NUM_THREADS BLIS_NUM_THREADS=$BLIS_NUM_THREADS srun -n1 --cpus-per-task=64 ./dgemm_$LIB.x "$SIZE" "$SIZE" "$SIZE"'>>'"$csvname"

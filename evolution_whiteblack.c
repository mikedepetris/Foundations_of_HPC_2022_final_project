...
void set_dead_or_alive_cell(const unsigned char *world, unsigned char *world_next, long world_size, long long int i) {
#ifdef DEBUG_ADVANCED_B
    printf("DEBUGB - set_dead_or_alive_cell 1 - mpi_rank=%d/%d, iteration_step=%d, i/local_size=%lld/%ld\n", mpi_rank, mpi_size, iteration_step, i, local_size);
#endif
    // actual cell coordinates
    long x = i % world_size;
    long y = i / world_size;
#ifdef DEBUG_ADVANCED_B
    printf("DEBUGB - set_dead_or_alive_cell 2 - mpi_rank=%d/%d, iteration_step=%d, i/local_size=%03lld/%ld x=%03ld, y=%03ld\n", mpi_rank, mpi_size, iteration_step, i, local_size, x, y);
#endif
    ...
    printf("DEBUGA - set_dead_or_alive_cell 3 - mpi_rank=%d/%d, iteration_step=%d, i/local_size=%03lld/%ld x=%03ld, y=%03ld sum=%d\n", mpi_rank, mpi_size, iteration_step, i, local_size, x, y, sum);
    //printf("DEBUGA - set_dead_or_alive_cell 3 - iteration_step=%d, i/world_size=%03lld/%ld x=%03ld, y=%03ld sum=%d\n", iteration_step, i, world_size, x, y, sum);
#endif
...
//#pragma omp master
    {
        ...
        if (mpi_rank == 0) {
#ifdef DEBUG_ADVANCED_B
            if (iteration_step == 1) {
                printf("DEBUGB - set_dead_or_alive_parallel_whiteblack 0a - mpi_rank=%d/%d, iteration_step=%d, world_local[0-7]=%d %d %d %d %d %d %d %d\n", mpi_rank, mpi_size
                ...
                for (long long i = 0; i < world_size * (local_size + 2); i++) {
                    if (i % 16 == 0)
                        printf("DEBUGB - set_dead_or_alive_parallel_whiteblack 0a - %08X: ", (unsigned int) i);
            ...
            if (iteration_step == 1) {
#ifdef DEBUG_ADVANCED_B
                printf("DEBUGB - set_dead_or_alive_parallel_whiteblack 0b - mpi_rank=%d/%d, iteration_step=%d, world_local[0-7]=%d %d %d %d %d %d %d %d\n", mpi_rank, mpi_size
                       , iteration_step, world_local[world_size], world_local[world_size + 1], world_local[world_size + 2], world_local[world_size + 3]
                       , world_local[world_size + 4], world_local[world_size + 5], world_local[world_size + 6], world_local[world_size + 7]);
#endif
                for (long long i = 0; i < world_size * (local_size + 2); i++) {
                    if (i % 16 == 0)
                        printf("DEBUGA - set_dead_or_alive_parallel_whiteblack 0b - %08X: ", (unsigned int) i);
                    //printf("%d ", world_local[i] == DEAD); 0/1
                    printf("%02X ", world_local[i]);
                    if (i % 16 == 15)
                        printf("\n");
...
#ifdef DEBUG_ADVANCED_MALLOC_FREE
            if (iteration_step == 1) {
                printf("DEBUGA - set_dead_or_alive_parallel_whiteblack 0c - mpi_rank=%d/%d, iteration_step=%d, world_local[0-7]=%d %d %d %d %d %d %d %d\n", mpi_rank, mpi_size
                       , iteration_step, world_local[world_size], world_local[world_size + 1], world_local[world_size + 2], world_local[world_size + 3]
                       , world_local[world_size + 4], world_local[world_size + 5], world_local[world_size + 6], world_local[world_size + 7]);
                for (long long i = 0; i < world_size * (local_size + 2); i++) {
                    if (i % 16 == 0)
                        printf("DEBUGA - set_dead_or_alive_parallel_whiteblack 0c - %08X: ", (unsigned int) i);
                    //printf("%d ", world_local[i] == DEAD); 0/1
                    printf("%02X ", world_local[i]);
                    if (i % 16 == 15)
                        printf("\n");
...
double evolution_whiteblack_parallel(const int mpi_rank, const int mpi_size, MPI_Status *mpi_status, MPI_Request *mpi_request, unsigned char **world_local, const long world_size, const long local_size, const int number_of_steps
    ...
    if (debug_info > 0)
        printf("DEBUG1 - evolution_whiteblack_parallel - BEGIN - mpi_rank=%d/%d, world_size=%ld\n", mpi_rank, mpi_size, world_size);
    ...
    for (int iteration_step = 1; iteration_step <= number_of_steps; iteration_step++) {
        ...
        if (iteration_step % number_of_steps_between_file_dumps == 0) {
            ...
            if (debug_info > 1)
                printf("DEBUG2 - evolution_whiteblack_parallel 1 - snap written mpi_rank=%d/%d, omp_rank=%d/%d, iteration_step=%d/%d\n", mpi_rank, mpi_size, omp_get_thread_num(), omp_get_max_threads(), iteration_step, number_of_steps);
        }
        // pointers swap to reuse allocated world_local and world_local_next for next iteration
        temp = world_local_actual;
        world_local_actual = world_local_next;
        world_local_next = temp;
        if (debug_info > 1)
            printf("DEBUG2 - evolution_whiteblack_parallel 2 - mpi_rank=%d/%d, omp_rank=%d/%d, iteration_step=%d/%d\n", mpi_rank, mpi_size, omp_get_thread_num(), omp_get_max_threads(), iteration_step, number_of_steps);
    }
    if (debug_info > 1)
        printf("DEBUG2 - evolution_whiteblack_parallel 3 - mpi_rank=%d/%d, omp_rank=%d/%d\n", mpi_rank, mpi_size, omp_get_thread_num(), omp_get_max_threads());
    free(world_local_next_original);
    free(image_filename_suffix);
    if (debug_info > 1)
        printf("DEBUG2 - evolution_whiteblack_parallel END - mpi_rank=%d/%d, omp_rank=%d/%d\n", mpi_rank, mpi_size, omp_get_thread_num(), omp_get_max_threads());
    return t_io;
}

double evolution_whiteblack_single(const int mpi_rank, const int mpi_size, MPI_Status *mpi_status, MPI_Request *mpi_request, unsigned char **world_local, const long world_size, const long local_size, const int number_of_steps
                                 , const int number_of_steps_between_file_dumps, const char *directoryname, int debug_info) {
    double t_io = 0; // returned value: total I/O time spent
    if (debug_info > 0)
...
#define MAX_STRING_LENGTH 256
    ...
    if (mpi_rank == 0) {
        ...
        if (debug_info > 0)
            printf("DEBUG1 - evolution_whiteblack 1 - rank %d/%d, len=%ld string_with_timestamp=%s\n", mpi_rank, mpi_size, len, string_with_timestamp);
        if (debug_info > 0)
    ...
    */
    ...
    if (debug_info > 0)
        printf("DEBUG1 - evolution_whiteblack 3 - rank %d/%d - maxval=%d, local_size=%ld, world_size=%ld, filename=%s\n", mpi_rank, mpi_size, maxval, local_size, world_size, filename);
    ...
    if (mpi_size > 1)
        t_io += evolution_whiteblack_parallel(mpi_rank, mpi_size, &mpi_status, &mpi_request, &world_local, world_size, local_size, number_of_steps, number_of_steps_between_file_dumps, directoryname, debug_info);
    else
        t_io += evolution_whiteblack_single(mpi_rank, mpi_size, &mpi_status, &mpi_request, &world_local, world_size, local_size, number_of_steps, number_of_steps_between_file_dumps, directoryname, debug_info);
    if (debug_info > 1)
        printf("DEBUG2 - evolution_whiteblack 4 - ITERATED - rank %d/%d - maxval=%d, local_size=%ld, world_size=%ld, directoryname=%s, filename=%s\n", mpi_rank, mpi_size, maxval, local_size, world_size, directoryname, filename);
    ...
    if (mpi_size > 1 && mpi_rank == 0) {
        ...
        for (int iteration_step = number_of_steps_between_file_dumps;
            ...
            if (debug_info > 1)
                printf("DEBUG2 - evolution_whiteblack 5a2 - rank %d/%d, LEN=%lu, snap_chunks_fn=%s/%s%03d_%05d%s.%s\n", mpi_rank, mpi_size, snap_fn_len, directoryname, IMAGE_FILENAME_PREFIX_SNAP_WHITEBLACK, mpi_size, iteration_step, ""
                       , FILE_EXTENSION_PGM);
...
//          sprintf(snap_fn, "%s/%s%03d_%05d%s.%s", directoryname, IMAGE_FILENAME_PREFIX_SNAP_WHITEBLACK, mpi_size, iteration_step, "", FILE_EXTENSION_PGM);
            ...
            if (debug_info > 1)
                printf("DEBUG2 - evolution_whiteblack 5a3 - snap_fn_len=%ld, strlen(%s)=%ld\n", snap_fn_len, snap_fn, strlen(snap_fn));
...
//              sprintf(snap_chunks_fn[i], "%s/%s%05d.%s", directoryname, IMAGE_FILENAME_PREFIX_SNAP_WHITEBLACK, iteration_step, FILE_EXTENSION_PGMPART);
                if (debug_info > 1)
                    printf("DEBUG2 - evolution_whiteblack 5a6: LEN=%lu %s\n", strlen(snap_chunks_fn[i]), snap_chunks_fn[i]);
            }
            if (debug_info > 0)
                for (int i = 0; i < mpi_size; i++)
                    printf("DEBUG1 - evolution_whiteblack 5a7: %s\n", snap_chunks_fn[i]);
            for (int i = 0; i < mpi_size; i++)
...
//      sprintf(final_fn, "%s/%s%03d%s.%s", directoryname, IMAGE_FILENAME_PREFIX_FINAL_WHITEBLACK, mpi_size, "", FILE_EXTENSION_PGM);
        ...
        if (debug_info > 1) {
            ...
            printf("DEBUG2 - evolution_whiteblack 5b0 - final_fn_len=%ld, strlen(%s)=%ld\n", final_fn_len, final_fn, strlen(final_fn));
            printf("DEBUG2 - evolution_whiteblack 5b1 - strlen(\"/000_000.\")=%ld\n", strlen("/000_000."));
            printf("DEBUG2 - evolution_whiteblack 5b2 - strlen(%s)=%ld\n", directoryname, strlen(directoryname));
            printf("DEBUG2 - evolution_whiteblack 5b3 - sizeof(%s)=%ld\n", IMAGE_FILENAME_PREFIX_FINAL_WHITEBLACK, sizeof(IMAGE_FILENAME_PREFIX_FINAL_WHITEBLACK));
            printf("DEBUG2 - evolution_whiteblack 5b4 - sizeof(%s)=%ld\n", FILE_EXTENSION_PGMPART, sizeof(FILE_EXTENSION_PGMPART));
        }
...
//          sprintf(final_chunks_fn[i], "%s/%s.%s", directoryname, IMAGE_FILENAME_PREFIX_FINAL_WHITEBLACK, FILE_EXTENSION_PGMPART);
            if (debug_info > 1)
                printf("DEBUG2 - evolution_whiteblack - JOIN1b: LEN=%lu %s\n", strlen(final_chunks_fn[i]), final_chunks_fn[i]);
        }
        if (debug_info > 0)
            for (int i = 0; i < mpi_size; i++)
                printf("DEBUG1 - evolution_whiteblack - JOIN2: %s\n", final_chunks_fn[i]);
        // delete if already existing, unnecessary as we create a new dir
        //int remove_result = remove(pathname);
        //if (debug_info > 0)
        //    printf("DEBUG1 - evolution_whiteblack - remove_result: %d\n", remove_result);
        for (int i = 0; i < mpi_size; i++)
        ...
        if (debug_info > 0)
            printf("DEBUG1 - evolution_whiteblack ACCU2 - t_io=%f, t_io_other=%f, t_io_accumulator=%f, mpi_size == 1 ? 0 : t_io_accumulator / (mpi_size - 1)=%f\n", t_io, t_io_other, t_io_accumulator, mpi_size == 1 ? 0 : t_io_accumulator / (mpi_size - 1));
    }

    if (mpi_rank == 0 && debug_info > 0)
        //DEBUG1 - evolution_whiteblack 6 - mpi=2, omp=2, time taken=0.007455
        printf("DEBUG1 - evolution_whiteblack 6 - mpi=%d, omp=%d, total time=%f, I/O time taken=%f\n", mpi_size, omp_get_max_threads(), MPI_Wtime() - t_start, t_io);
    MPI_Finalize();
    free(directoryname);
    if (debug_info > 1)
        printf("DEBUG2 - evolution_whiteblack 7 - rank %d/%d, filename=%s\n", mpi_rank, mpi_size, filename);
    free(world_local);
    if (debug_info > 1)
        printf("DEBUG2 - evolution_whiteblack 8 - rank %d/%d, filename=%s\n", mpi_rank, mpi_size, filename);
    if (mpi_rank == 0)
    ...
    if (debug_info > 0)
        printf("DEBUG1 - evolution_whiteblack END - rank %d/%d, filename=%s\n", mpi_rank, mpi_size, filename);
}

...
#pragma omp for schedule(static, 1)
    for (int i = 0; i < size_next_update; i++) {
        ...
        //Determine the rows and columns of the neighbours
...
#pragma omp for
    for (int i = 0; i < it; i++) {
        next_update[i]->row = i;
        next_update[i]->col = it;
        next_update[i + it]->row = it;
        next_update[i + it]->col = i;
    }
#pragma omp master
    {
...
#pragma omp master
                {
                    ...
                    write_pgm_image_complete(world, 255, world_size, world_size, fname);
                }
#pragma omp barrier
            }

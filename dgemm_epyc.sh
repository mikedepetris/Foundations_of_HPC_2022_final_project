...
for REP in {1..10}; do
  for SIZE in {2000..20000..500}; do
    ...
    srun -n1 --cpus-per-task=64 ./dgemm_mkl.x "$SIZE" "$SIZE" "$SIZE" >>"$csvname"
    #srun -n1 --cpus-per-task=64 ./dgemm_mkl.x 2000 2000 2000
  done
done
...
for REP in {1..10}; do
  for SIZE in {2000..20000..500}; do
    echo rep "$REP" size "$SIZE"
    srun -n1 --cpus-per-task=64 ./dgemm_mkl_optimized.x "$SIZE" "$SIZE" "$SIZE" >>"$csvname"
    #srun -n1 --cpus-per-task=64 ./dgemm_mkl_optimized.x 2000 2000 2000
  done
done
...
for REP in {1..10}; do
  for SIZE in {2000..20000..500}; do
    echo rep "$REP" size "$SIZE"
    srun -n1 --cpus-per-task=64 ./dgemm_oblas.x "$SIZE" "$SIZE" "$SIZE" >>"$csvname"
...
for REP in {1..10}; do
  for SIZE in {2000..20000..500}; do
    echo rep "$REP" size "$SIZE"
    srun -n1 --cpus-per-task=64 ./dgemm_oblas_optimized.x "$SIZE" "$SIZE" "$SIZE" >>"$csvname"
    #  srun -n1 --cpus-per-task=64 ./dgemm_oblas_optimized.x 2000 2000 2000
  done
done
